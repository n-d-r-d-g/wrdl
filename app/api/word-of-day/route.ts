...
async function detectWordOfTheDay(): Promise<string> {
  ...
  try {
    ...
    await page.waitForSelector('[data-testid="wordle-app-game"]', { timeout: 10000 });
    ...
    for (const word of strategicWords) {
      try {
        ...
        const gameWon = await page.evaluate(() => {
          const toastElement = document.querySelector('[data-testid="toast-message"]');
          return toastElement?.textContent?.includes('Magnificent') || 
